...
*   **控制模式**：显式切换并回读位置模式或电流模式；上电默认位置模式。
*   **零点设置**：将当前位置设置为新的零点。
...
│   ├── 13_demo_control_mode.py      # 位置/电流模式安全切换
│   ├── 14_demo_force_feedback_control.py # 遥操/力反馈公开接口验证
│   └── 15_demo_direct_current.py    # 指定关节电流输入与六轴电流监视
```
